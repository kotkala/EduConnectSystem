  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center space-x-2">
        ...
        />
        <Label htmlFor="is_subject_combination">Lớp chuyÃªn mÃ´n</Label>
      </div>

      {watchIsSubjectCombination && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-6">
          <div className="space-y-2">
            <Label htmlFor="subject_combination_type">Loáº¡i chuyÃªn mÃ´n *</Label>
            <Select
              value={(form.watch("subject_combination_type") as string) || ""}
              onValueChange={(value) => form.setValue("subject_combination_type", value)}
            >
              <SelectTrigger>
                <SelectValue placeholder="Chồn loại chuyÃªn mÃ´n" />
              </SelectTrigger>
              <SelectContent>
          ...
          {watchSubjectCombinationType && (
            <div className="space-y-2">
              <Label htmlFor="subject_combination_variant">Biáº¿n thể *</Label>
              <Select
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder="Chồn biáº¿n thể" />
                </SelectTrigger>
                <SelectContent>
...
function ClassFormComponent({ class: classData, onSuccess, onCancel, defaultAcademicYearId }: Readonly<ClassFormProps>) {
  ...
  // Form data (removed academicYears - now from global context)
  ...
  // Load form data (now filtered by selected academic year)
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const data = await loadFormData(effectiveAcademicYearId);
      ...
      <div className="grid gap-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id="name"
            {...form.register("name")}
            placeholder={watchAutoGeneratedName ? "Will be auto-generated" : "Enter class name (e.g., 10A1, Lý-HÃ³a-Sinh-01)"}
        ...
        {/* Academic Year - Display Only (from global context) */}
        <div className="space-y-2">
          <Label htmlFor="academic_year_display">Năm hồc *</Label>
          <div className="flex items-center gap-2 p-3 border rounded-md bg-muted/50">
            <span className="text-sm font-medium">
              {selectedAcademicYear?.name || 'Chưa chồn năm hồc'}
            </span>
          ...
          <p className="text-xs text-muted-foreground">
            Năm hồc Ä‘Æ°á»£c chồn từ dropdown á»Ÿ gÃ³c pháº£i trÃªn cÃ¹ng
          </p>
        </div>

        {/* Semester */}
        <div className="space-y-2">
          <Label htmlFor="semester_id">Hồc kỳ *</Label>
          <Select
          ...
          >
            <SelectTrigger className={form.formState.errors.semester_id ? "border-red-500" : ""}>
              <SelectValue placeholder="Chồn hồc kỳ" />
            </SelectTrigger>
            <SelectContent>
              {semesters.map((semester) => (
                <SelectItem key={semester.id} value={semester.id}>
                  {semester.name} ({semester.weeks_count} tuần) {semester.is_current && "(Hiá»‡n táº¡i)"}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                {isEditing ? "Äang cập nhật..." : "Äang tạo..."}
              </>
            ) : (
              <>
