...
export async function getStudentTimetableAction(filters: StudentTimetableFilters = {}) {
  try {
    ...
    if (classError) {
      ...
      throw new Error('Lỗi khi tải thông tin lớp hồc')
    }

    if (!classAssignments || classAssignments.length === 0) {
      console.error('No class assignments found for student:', userId)
      throw new Error('Báº¡n chưa Ä‘Æ°á»£c phÃ¢n công vÃ o lớp hồc nÃ o. Vui lòng liên hồ‡ về›i ban giám hiệu Ä‘á»ƒ Ä‘Æ°á»£c hồ— trá»£.')
    }
    ...
    if (timetableError) {
      ...
      throw new Error('Lỗi khi tải thồi khóa biểu')
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Không thể tải thồi khóa biểu'
    }
  }
}
...
export async function getStudentGradesAction() {
  try {
    ...
    if (!classAssignments || classAssignments.length === 0) {
      throw new Error('Báº¡n chưa Ä‘Æ°á»£c phÃ¢n công vÃ o lớp hồc nÃ o.')
    }
    ...
    if (gradesError) {
      ...
      throw new Error('Lỗi khi tải Ä‘iá»ƒm sá»‘')
    }
...
export async function getStudentGradeSummaryAction() {
  try {
    ...
    if (summaryError) {
      throw new Error('Lỗi khi tải tá»•ng hợp Ä‘iá»ƒm sá»‘')
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Không thể tải tá»•ng hợp Ä‘iá»ƒm sá»‘'
    }
  }
}
