...
import { getMonthlyRankingAction } from '@/lib/actions/violation-actions'
...
export default function MonthlyViolationSummary() {
  ...
  const handleMarkAsViewed = async (summaryId: string) => {
    try {
      ...
      const { markMonthlyAlertSeenAction } = await import('@/lib/actions/violation-actions')
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Xếp hạng học sinh theo số lần vi phạm trong tháng (4 tuần) - Cảnh báo khi ≥3 vi phạm
          </CardDescription>
        </CardHeader>
        <CardContent>
