          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                ...
                {request.target_name && ` → ${request.target_name}`}
              </span>
            </div>
  ...
  const renderExchangeDetails = (req: ExchangeRequest) => (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
        <div>
          <strong>Subject:</strong> {req.subject_code} - {req.subject_name}
        </div>
        <div>
          <strong>Class:</strong> {req.class_name}
        </div>
        <div>
          <strong>Time:</strong> {dayNames[req.day_of_week]} {req.start_time}-{req.end_time}
        </div>
        <div>
          <strong>Classroom:</strong> {req.classroom_name}
        </div>
        <div>
          <strong>Week:</strong> {req.week_number}
        </div>
        <div>
          <strong>Exchange Date:</strong> {format(new Date(req.exchange_date), 'PPP')}
        </div>
      </div>
