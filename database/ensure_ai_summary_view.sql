-- Ensure AI summary view exists for parent feedback
-- This script can be run safely multiple times

-- First, ensure the AI summary columns exist in feedback_notifications table
...
-- Add index for AI summary queries if not exists
...
-- Create or replace the view for parent feedback with AI summary support
...
-- Create RLS policy for the view if not exists (parents can only see their own children's feedback)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'feedback_notifications' 
        AND policyname = 'Parents can view their children feedback with AI summary'
    ) THEN
        CREATE POLICY "Parents can view their children feedback with AI summary" ON feedback_notifications
        FOR SELECT USING (
...
SELECT 'View parent_feedback_with_ai_summary created successfully' as status;
