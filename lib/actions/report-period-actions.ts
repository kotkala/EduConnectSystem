...
export async function adminBulkSendReportsAction(reportPeriodId: string) {
  try {
    ...
    console.log(`📊 Debug: Found ${allReports?.length || 0} total reports for period ${reportPeriodId}`)
...
export async function sendParentRemindersAction(reportPeriodId: string) {
  try {
    ...
    //             if (result.success) {
    //               console.log(`✅ Reminder email sent to parent ${parent.email} for student ${student.full_name}`)
    //             } else {
    //               console.error(`❌ Failed to send reminder to parent ${parent.email}:`, result.error)
    //             }
    //           } catch (error) {
    //             console.error(`❌ Failed to send reminder to parent ${parent.email}:`, error)
    //           }
    //         })()
    //       )
    //     }
...
export async function resetReportsToDraftAction(reportPeriodId: string) {
  try {
    ...
    console.log(`✅ Reset ${sentReports.length} reports from 'sent' to 'draft' status`)
