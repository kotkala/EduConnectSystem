...
export function StudentForm({ onSubmit, onCancel, loading, parents }: StudentFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    // Validate
    if (!form.phone.trim() || !form.full_name.trim() || !form.email.trim() || !form.password || !form.street_address.trim() || !form.city.trim() || !form.province.trim() || !form.parent_id) {
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <select id="parent_id" name="parent_id" value={form.parent_id} onChange={handleChange} required disabled={loading} className="w-full border rounded px-3 py-2">
