      ...
      {watchIsSubjectCombination && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-6">
          <div className="space-y-2">
            ...
            <Select
              value={(watchSubjectCombinationType as string) || ""}
          ...
          {watchSubjectCombinationType && (
            <div className="space-y-2">
              ...
              <Select
                value={(watchSubjectCombinationType && SUBJECT_COMBINATIONS[watchSubjectCombinationType as keyof typeof SUBJECT_COMBINATIONS]?.variants?.find(v => v.id === form.watch("subject_combination_variant"))?.id) || ""}
...
function ClassFormComponent({ class: classData, onSuccess, onCancel, defaultAcademicYearId }: Readonly<ClassFormProps>) {
  ...
  // Watch for form changes using Context7 patterns - Optimized with useMemo
  const formValues = form.watch()
  const watchIsSubjectCombination = useMemo(() => formValues.is_subject_combination, [formValues.is_subject_combination])
  const watchSubjectCombinationType = useMemo(() => formValues.subject_combination_type, [formValues.subject_combination_type])
  const watchAutoGeneratedName = useMemo(() => formValues.auto_generated_name, [formValues.auto_generated_name])
  const watchClassBlockId = useMemo(() => formValues.class_block_id, [formValues.class_block_id])
  const watchClassSuffix = useMemo(() => formValues.class_suffix, [formValues.class_suffix])
  ...
  const loadInitialData = useCallback(async () => {
    try {
      // Load only essential data - reduced limit for academic years
      const academicYearsResult = await getAcademicYearsAction({ page: 1, limit: 20 })
      ...
      // Load teachers and class blocks in parallel - optimized
  ...
  // Optimized semester loading with useCallback and memoization
  ...
  // Load semesters when academic year changes - optimized
  useEffect(() => {
    const selectedAcademicYearId = formValues.academic_year_id
    if (selectedAcademicYearId) {
      loadSemesters(selectedAcademicYearId)
    } else {
      setSemesters([])
    }
  }, [formValues.academic_year_id, loadSemesters])
      ...
      <div className="grid gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="auto_generated_name">Sử dụng tiền tố mặc định (Ví dụ, Khối 10 + A1 = 10A1)</Label>
          </div>
          ...
          {watchAutoGeneratedName && (
            <div className="grid grid-cols-2 gap-4 p-4 border rounded-lg bg-gray-50">
              <div className="space-y-2">
                <Label htmlFor="class_block_id">Khối lớp *</Label>
                <Select
              ...
              <div className="space-y-2">
                <Label htmlFor="class_suffix">Hậu tố *</Label>
                <Input
                  id="class_suffix"
        ...
        {/* Academic Year - Dropdown Selection - Optimized */}
        <div className="space-y-2">
        ...
        {/* Homeroom Teacher - Only show for non-subject combination classes - Optimized */}
        {!watchIsSubjectCombination && (
          <div className="space-y-2">
            <Label htmlFor="homeroom_teacher_id">Giáo viên chủ nhiệm</Label>
            <Select
              value={formValues.homeroom_teacher_id || "none"}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="none">Không có giáo viên chủ nhiệm</SelectItem>
        ...
        {/* Max Students */}
        <div className="space-y-2">
          <Label htmlFor="max_students">Tổng số học sinh *</Label>
          <Input
            id="max_students"
            type="number"
            min="1"
