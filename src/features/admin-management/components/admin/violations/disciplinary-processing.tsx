...
export default function DisciplinaryProcessing() {
  ...
  const sendCaseToHomeroom = async (caseId: string) => {
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.error || 'Có lỗi xảy ra khi gá»­i case')
      }
    } catch (error) {
      console.error('Lỗi gá»­i case:', error)
      toast.error('Có lỗi xảy ra khi gá»­i case')
    }
  }
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('ÄÃ£ cập nhật hình thức kỷ luật')
        ...
        toast.error(res.error || 'Cập nhật tháº¥t báº¡i')
      }
    } catch (error) {
  ...
  const handleDeleteSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success('ÄÃ£ vÃ´ hiệu hÃ³a hình thức kỷ luật')
        setActionTypes(prev => prev.map(t =>
        ...
        toast.error(res.error || 'Thao tác tháº¥t báº¡i')
      }
    } catch (error) {
  ...
  const handleCreateActionType = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        toast.success('ÄÃ£ tạo hình thức kỷ luật mới')
      } else {
  ...
  const handleCreateCase = async () => {
    if (!selectedStudent || !selectedActionType || !caseDescription.trim()) {
      toast.error('Vui lòng Ä‘iá»n Ä‘áº§y Ä‘á»§ thông tin')
      return
    }
    ...
    try {
      ...
      const current = semestersRes.success && semestersRes.data
        ...
        : null
      if (!current) {
        toast.error('Không tìm thấy hồc kì hiá»‡n táº¡i')
        return
      }
      ...
      if (res.success) {
        toast.success('ÄÃ£ tạo case kỷ luật mới')
        setCaseDescription('')
        setSelectedStudent('')
  ...
  const getStatusBadge = (status: string) => {
    const statusMap = {
      ...
      acknowledged: { variant: 'default' as const, label: 'GVCN Ä‘Ã£ xác nhận' },
      meeting_scheduled: { variant: 'default' as const, label: 'ÄÃ£ lÃªn lịch hồp' },
      resolved: { variant: 'default' as const, label: 'ÄÃ£ giáº£i quyết' }
    }
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Xá»­ lý kỷ luật vi phạm
          </CardTitle>
          <CardDescription>
            Quản lý hình thức kỷ luật vÃ  tạo case xá»­ lý cho hồc sinh vi phạm
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="view-cases">Xem táº¥t cả case</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="add-case" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Chồn hồc sinh vÃ  hình thức kỷ luật phù hợp
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  <Label>Khồ‘i lớp</Label>
                  <Select value={selectedBlock} onValueChange={setSelectedBlock}>
                    <SelectTrigger>
                      <SelectValue placeholder="Chồn khồ‘i" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="10">Khồ‘i 10</SelectItem>
                      <SelectItem value="11">Khồ‘i 11</SelectItem>
                      <SelectItem value="12">Khồ‘i 12</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="space-y-2">
                <Label>Hồc sinh</Label>
                <Select value={selectedStudent} onValueChange={setSelectedStudent}>
                  <SelectTrigger>
                    <SelectValue placeholder="Chồn hồc sinh" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="student1">Nguyá»…n VÄƒn An (SU0001)</SelectItem>
                    <SelectItem value="student2">Tráº§n Thồ‹ BÃ¬nh (SU002)</SelectItem>
              ...
              <div className="space-y-2">
                <Label>Ná»™i dung cụ thể vi phạm</Label>
                <Textarea
                  placeholder="Mô tả chi tiết các vi phạm vÃ  lý do Ã¡p dá»¥ng hình thức kỷ luật nÃ y..."
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label>Mức Ä‘á»™ nghiÃªm trá»ng</Label>
                  <Select 
              ...
              <div className="space-y-2">
                <Label>Mô tả</Label>
                <Textarea
                  placeholder="Mô tả chi tiết về hình thức kỷ luật nÃ y..."
                    ...
                    <TableRow key={type.id}>
                      ...
                      <TableCell>
                        <Badge variant={type.is_active ? 'default' : 'secondary'}>
                          {type.is_active ? 'Hoạt Ä‘á»™ng' : 'Táº¡m dá»«ng'}
                        </Badge>
                      </TableCell>
                ...
                <TableBody>
                  {cases.map((caseItem) => (
                    <TableRow key={caseItem.id}>
                      <TableCell>
                        <div>
                          <div className="font-medium">{caseItem.student?.full_name || 'Không xác Ä‘á»‹nh'}</div>
                          <div className="text-sm text-muted-foreground">{caseItem.student?.student_id || 'N/A'}</div>
                        </div>
                      </TableCell>
                      <TableCell>{caseItem.class?.name || 'Không xác Ä‘á»‹nh'}</TableCell>
                      <TableCell>
                        <div>
                          <div className="font-medium">{caseItem.action_type?.name || 'Không xác Ä‘á»‹nh'}</div>
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="edit-severity">Mức Ä‘á»™ nghiÃªm trá»ng</Label>
              <Select
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="1">Mức 1 - Nháº¹</SelectItem>
                  <SelectItem value="2">Mức 2 - Trung bÃ¬nh</SelectItem>
                  <SelectItem value="3">Mức 3 - Náº·ng</SelectItem>
                  <SelectItem value="4">Mức 4 - Ráº¥t náº·ng</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
      ...
      <Dialog open={deleteModalOpen} onOpenChange={setDeleteModalOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Xác nhận vÃ´ hiệu hÃ³a</DialogTitle>
            <DialogDescription>
              Báº¡n có cháº¯c cháº¯n muá»‘n vÃ´ hiệu hÃ³a hình thức kỷ luật &quot;{deletingActionType?.name}&quot;?
              HÃ nh Ä‘á»™ng nÃ y không thể hoÃ n tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
