...
export default function ViolationRecordForm({ onSuccess }: Readonly<ViolationRecordFormProps>) {
  ...
  useEffect(() => {
    if (watchedClassId) {
      ...
      if (selectedClass) {
        ...
        form.setValue('semester_id', '62f2a9ae-8aeb-43c6-ba14-17f7b82ce609') // Hồc kỳ 1
      }
    } else {
      setStudents([])
      setSelectedStudents([])
  ...
  const onSubmit = async (data: BulkStudentViolationFormData) => {
    try {
      ...
      if (result.success) {
        toast.success(`Ghi nhận vi phạm thÃ nh công cho ${data.student_ids.length} hồc sinh`)
        ...
        toast.error(result.error || 'Ghi nhận vi phạm tháº¥t báº¡i')
      }
    } catch {
