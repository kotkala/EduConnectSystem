...
export async function getTeacherHomeroomClassesAction(): Promise<{
  ...
  try {
    ...
    if (!profile || profile.role !== 'teacher') {
      throw new Error("Từ chồ‘i truy cập. Yêu cầu vai trÃ² giáo viên.")
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể láº¥y danh sách lớp chủ nhiệm"
    }
  }
}
...
export async function getHomeroomStudentsWithParentsAction(classId: string): Promise<{
  ...
  try {
    ...
    if (!classInfo || classInfo.homeroom_teacher_id !== user.id) {
      throw new Error("Từ chồ‘i truy cập. Báº¡n không pháº£i lÃ  giáo viên chủ nhiệm của lớp nÃ y.")
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể láº¥y danh sách hồc sinh vÃ  phụ huynh"
    }
  }
}
...
export async function createMeetingScheduleAction(request: CreateMeetingScheduleRequest): Promise<{
  ...
  try {
    ...
    if (meetingError || !meetingSchedule) {
      throw new Error(meetingError?.message || "Không thể tạo lịch háº¹n")
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể tạo lịch háº¹n"
    }
  }
}
...
export async function getParentMeetingSchedulesAction(): Promise<{
  ...
  try {
    ...
    if (!profile || profile.role !== 'parent') {
      throw new Error("Từ chồ‘i truy cập. Yêu cầu vai trÃ² phụ huynh.")
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể láº¥y danh sách lịch háº¹n"
    }
  }
}
...
export async function markMeetingScheduleAsReadAction(meetingScheduleId: string): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể Ä‘Ã¡nh dấu Ä‘Ã£ Ä‘á»c"
    }
  }
}
...
export async function getUnreadMeetingScheduleCountAction(): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Không thể láº¥y sá»‘ lÆ°á»£ng chưa Ä‘á»c"
    }
  }
}
