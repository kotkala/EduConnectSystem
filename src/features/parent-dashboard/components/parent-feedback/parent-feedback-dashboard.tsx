...
export default function ParentFeedbackDashboard() {
  ...
  const renderTeacherAiSummary = useCallback((dayFeedback: StudentFeedbackForParent['daily_feedback'][string], dayOfWeek: number, student: StudentFeedbackForParent) => {
    // First check for daily AI summary from daily_feedback_summaries table
    const dailyAiSummary = student.daily_ai_summaries?.[dayOfWeek.toString()]

    // Fallback to individual feedback AI summary
    const teacherAiSummary = dayFeedback.find(f => f.ai_summary && f.use_ai_summary)

    const aiSummary = dailyAiSummary || teacherAiSummary
      ...
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20 rounded-lg p-3 border border-green-200/50 dark:border-green-800/50 mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-xs text-green-600 dark:text-green-400">
            {dailyAiSummary
              ? new Date(dailyAiSummary.created_at).toLocaleDateString('vi-VN')
        ...
        <div className="bg-white dark:bg-gray-900 rounded-md p-2 border">
          <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">
            {dailyAiSummary ? dailyAiSummary.ai_summary : teacherAiSummary!.ai_summary}
          </p>
        </div>
                      ...
                      <CardContent>
                        {dayFeedback.length > 0 ? (
                          <div className="space-y-4">
                            ...
                            {showDailyAiSummary && dayFeedback.length > 0 && renderTeacherAiSummary(dayFeedback, dayOfWeek, student)}
