...
export async function saveAIFeedbackAction(request: SaveAIFeedbackRequest): Promise<{
  ...
  try {
    ...
    if (submissionError || !submission) {
      throw new Error("KhÃ´ng tÃ¬m tháº¥y bÃ i ná»™p hoáº·c bá»‹ tá»« chá»‘i truy cáº­p")
    }
...
export async function getAIFeedbackAction(submissionId: string, studentId: string): Promise<{
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "KhÃ´ng thá»ƒ láº¥y pháº£n há»“i AI"
    }
  }
}
