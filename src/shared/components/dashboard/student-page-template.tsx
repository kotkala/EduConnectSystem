...
export function StudentPageTemplate({
  ...
  backLink,
  actions,
  breadcrumbs = [{ label: "Bảng điều khiển", href: "/dashboard/student" }],
  showCard = true
  ...
  return (
    <ContentLayout title={title} role="student">
      <Breadcrumb>
        <BreadcrumbList>
...
interface StudentPageWithSuspenseProps extends StudentPageTemplateProps {
  fallback?: React.ReactNode
}

export function StudentPageWithSuspense({
  fallback = <div>Đang tải...</div>,
  ...props
}: StudentPageWithSuspenseProps) {
  return (
    <StudentPageTemplate {...props}>
